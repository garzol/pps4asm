...
creation 
2024-04-12
inversion bit to bit just to see and creation of specific py file for that
'''
...
def entryPoint(fn):   
    ...
    if path.exists(basefilename+"_inv.coe"):
    ...
    print (f"Will generate {basefilename}_inv.coe")   
    title =f";******************************************************************\n\
...
#     f.close()        
    f = open(basefilename+"_inv.coe", "w")
    f.write(title)
    f.write(radix)
    f.write(head)
    i=0
        ...
        while b != b"":
            ...
            for ri in range(4):  
                ...
                if int.from_bytes(b, "big")&(1<<ri) != 0:
                    f.write("0")   #inverse of standard case
                else:
                    f.write("1")   #inverse of standard case
